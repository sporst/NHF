...
public class BitTranslator
{
	public static List<ReilInstruction> translate(final StandardEnvironment environment, final Instruction instruction)
	{
		...
		instructions.add(ReilHelpers.createAnd(offset++, OperandSize.BYTE, operandResult.getResultRegister(), OperandSize.BYTE, "A", OperandSize.BYTE, andResult));

		instructions.addAll(FlagTranslator.translateZ(environment, baseOffset + instructions.size(), andResult));
		instructions.addAll(FlagTranslator.translateN(environment, baseOffset + instructions.size(), andResult));
